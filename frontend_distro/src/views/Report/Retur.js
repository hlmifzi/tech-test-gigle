...
import { GetReportr, EmptyDataSourceAction } from './ZActionReport'
...
class Retur extends Component {
  ...
  handleSave = async (date,dateString) => {
    ...
    try {
      await this.props.GetReportr(request,'laporan/retur')
    } catch (error) {
      alert(error.message)
    }
  };
      ...
      <div className="animated fadeIn">
        <Row>
          <Col xl={12}>
            <Card>
              <CardHeader>
                <i className="nav-icon icon-action-undo"></i> Report Retur
              </CardHeader>
              <CardBody>
                <div className="row">
...
const mapDispatchToProps = { GetReportr , EmptyDataSourceAction}
