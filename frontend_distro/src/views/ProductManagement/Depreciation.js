...
import { ProductSelectGetList, DepreciationAddAction } from './TransactionAction'
...
class Depreciation extends Component {
  ...
  handleSave = async () => {
    ...
    let EntryAdd = {}
    try {
      EntryAdd = await this.props.DepreciationAddAction(payload)
    } catch (error) {
      alert(error.massage)
    }
  ...
  render() {
    ...
    let Product = Products.map((v, i) =>
      (v.type === "NORMAL") ? <option key={i} value={v.key}> {v.Article} - {v.product_name} - {v.type}</option> : ''
    )
...
const mapDispatchToProps = { ProductSelectGetList, GetListAllTenants, DepreciationAddAction }
