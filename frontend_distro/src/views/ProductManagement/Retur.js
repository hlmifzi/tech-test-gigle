...
import { ProductSelectGetList, ReturAddAction } from './TransactionAction'
...
class Retur extends Component {
  ...
  handleSave = async () => {
    ...
    let EntryAdd = {}
    try {
      EntryAdd = await this.props.ReturAddAction(payload)
    } catch (error) {
      alert(error.massage)
    }
  ...
  render() {
    const {
      icon,
      modul
    } = this.props.TransactionReducer.Retur
                ...
                <Row>
                  ...
                  <Col xs="4">
                    <FormGroup>
                      ...
                      <Input type="select" onChange={(e) => this.setState({ place_to: e.target.value })} name="ccmonth" id="tenantTo" >
                        <option value="0"> Choose Gudang</option>
                        {Tenant}
                      </Input>
                    </FormGroup>
...
const mapDispatchToProps = { ProductSelectGetList, GetListAllTenants, ReturAddAction }
const connectRedux = connect(mapStateToProps, mapDispatchToProps)(Retur)
